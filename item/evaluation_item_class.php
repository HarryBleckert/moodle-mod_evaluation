...
abstract class evaluation_item_base {
     ...
     */
    abstract public function build_editform($item, $evaluation, $cm);
    ...
    public function edit_actions($item, $evaluation, $cm) {
        ...
        if ($this->can_switch_require()) {
            if ($item->required == 1) {
                $buttontitle = get_string('switch_item_to_not_required', 'evaluation');
                $buttonimg = 'required';
            } else {
                $buttontitle = get_string('switch_item_to_required', 'evaluation');
