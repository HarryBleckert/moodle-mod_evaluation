...
namespace mod_evaluation\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('evaluation_valuetmp', $valuefields, 'privacy:metadata:valuetmp');
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        $sql = "
            SELECT DISTINCT ctx.id
              FROM {%s} fc
              JOIN {modules} m
                ON m.name = :evaluation
              JOIN {course_modules} cm
                ON cm.instance = fc.evaluation
               AND cm.module = m.id
              JOIN {context} ctx
                ON ctx.instanceid = cm.id
               AND ctx.contextlevel = :modlevel
             WHERE fc.userid = :userid";
        $params = ['evaluation' => 'evaluation', 'modlevel' => CONTEXT_MODULE, 'userid' => $userid];
        $contextlist = new contextlist();
        $contextlist->add_from_sql(sprintf($sql, 'evaluation_completed'), $params);
        $contextlist->add_from_sql(sprintf($sql, 'evaluation_completedtmp'), $params);
        return $contextlist;
    }
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        // Find users with evaluation entries.
        $sql = "
            SELECT fc.userid
              FROM {%s} fc
              JOIN {modules} m
        ...
        $params = ['evaluation' => 'evaluation', 'modlevel' => CONTEXT_MODULE, 'contextid' => $context->id];

        $userlist->add_from_sql('userid', sprintf($sql, 'evaluation_completed'), $params);
        $userlist->add_from_sql('userid', sprintf($sql, 'evaluation_completedtmp'), $params);
    }
        ...
        $completedsql = "
            SELECT fc.id
              ...
              JOIN {course_modules} cm
               ...
               AND cm.module = m.id
             WHERE cm.id = :cmid";
        $completedparams = ['cmid' => $context->instanceid, 'evaluation' => 'evaluation'];

        // Delete temp answers and submissions.
        $completedtmpids = $DB->get_fieldset_sql(sprintf($completedsql, 'evaluation_completedtmp'), $completedparams);
        if (!empty($completedtmpids)) {
            list($insql, $inparams) = $DB->get_in_or_equal($completedtmpids, SQL_PARAMS_NAMED);
            $DB->delete_records_select('evaluation_valuetmp', "completed $insql", $inparams);
            $DB->delete_records_select('evaluation_completedtmp', "id $insql", $inparams);
        }

        // Delete answers and submissions.
        $completedids = $DB->get_fieldset_sql(sprintf($completedsql, 'evaluation_completed'), $completedparams);
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        $completedsql = "
            SELECT fc.id
             ...
             WHERE fc.userid = :userid
               AND cm.id $insql";
        $completedparams = array_merge($inparams, ['userid' => $userid, 'evaluation' => 'evaluation']);
        ...
        $completedsql = "
            SELECT fc.id
             ...
             WHERE cm.id = :instanceid
               AND fc.userid $insql";
        $completedparams = array_merge($inparams, ['instanceid' => $context->instanceid, 'evaluation' => 'evaluation']);
