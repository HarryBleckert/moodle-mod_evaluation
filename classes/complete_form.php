...
/**
 * Contains class mod_evaluation_complete_form
 *
...
/**
 * Class mod_evaluation_complete_form
 *
 * @package   mod_evaluation
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evaluation_complete_form extends moodleform {
     ...
     */
    public function __construct($mode, mod_evaluation_structure $structure, $formid, $customdata = null) {
        $this->mode = $mode;
    ...
    public function definition() {
        ...
        if (isloggedin() && !isguestuser() && $this->mode != self::MODE_EDIT && $this->mode != self::MODE_VIEW_TEMPLATE &&
            ...
            if ($this->structure->is_anonymous()) {
                ...
                $anonymousmodeinfo = get_string('non_anonymous', 'evaluation');
            }
            $element = $mform->addElement('static', 'anonymousmode', '',
                    get_string('mode', 'evaluation') . ': ' . "<b>$anonymousmodeinfo</b>");
        ...
        if ($this->mode == self::MODE_COMPLETE) {
            $buttonarray = array();
            $buttonarray[] = &$mform->createElement('submit', 'gopreviouspage', get_string('previous_page', 'evaluation'));
            $buttonarray[] = &$mform->createElement('submit', 'gonextpage', get_string('next_page', 'evaluation'),
                    array('class' => 'form-submit'));
            $buttonarray[] = &$mform->createElement('submit', 'savevalues', get_string('save_entries', 'evaluation'),
                    array('class' => 'form-submit'));
    ...
    public function display() {
        ...
        if ($this->mode == self::MODE_EDIT) {
            $PAGE->requires->js_call_amd('mod_evaluation/edit', 'setup');
        }
    }
}
