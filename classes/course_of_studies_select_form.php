...
class mod_evaluation_course_of_studies_select_form extends moodleform {
    ...
    public function definition() {
        ...
        if (!$this->_form->_freezeAll && ($courses = $evaluationstructure->get_completed_course_of_studies()) && count($courses) ) 
		{
            $elements = [];
            $elements[] = $mform->createElement('autocomplete', 'course_of_studiesID', get_string('filter_by_course_of_studies', 'evaluation'),
		...
		$this->set_data([ 'course_of_studiesID' => $evaluationstructure->get_course_of_studiesID(), 'id' => $evaluationstructure->get_cm()->id]);
    }
}
