...
/**
 * Contains class mod_evaluation_completion
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evaluation_completion extends mod_evaluation_structure {
    /** @var stdClass */
    protected $completed;
    /** @var stdClass */
    ...
    /** @var mod_evaluation_complete_form the form used for completing the evaluation */
    protected $form = null;
	...
	//protected $course_of_studies = $_SESSION['course_of_studies'];
     ...
     * @param cm_info $cm course module object corresponding to the $evaluation
	 ...
	 * @param int $teacherid to use teamteaching ability to make 1 evaluation/teacher. Set to 0 if unused.
     */
    public function __construct($evaluation, $cm, $courseid, $iscompleted = false, $completedid = null,
                                $nonanonymouseuserid = null, $userid = 0, $teacherid = 0, $course_of_studies = "" ) {
        global $DB;

        parent::__construct($evaluation, $cm, $courseid, 0, $userid, $teacherid, $course_of_studies );
        ...
        if ($iscompleted) {
            ...
            if (!$nonanonymouseuserid && !$completedid) {
                throw new coding_exception('Either $completedid or $nonanonymouseuserid must be specified for completed evaluations');
            }
            if ($completedid) {
            ...
            $this->completed = $DB->get_record('evaluation_completed', $params, '*', MUST_EXIST);
            $this->courseid = $this->completed->courseid;
			$this->teacherid = $this->completed->teacherid;
			$this->course_of_studies = $this->completed->course_of_studies;
        }
    }
    ...
    public function get_current_completed_tmp() {
        ...
        if ($this->completedtmp === null) {
			...
			if ($teacherid = $this->get_teacherid()) 
			{	$params['teacherid'] = $teacherid;
				//print "<br><br>Teacher ID: \n";var_dump($teacherid);print "<br><br>\n";
            }
            ...
            $this->completedtmp = $DB->get_record('evaluation_completedtmp', $params);
        }
    ...
    public function get_unfinished_responses() {
        ...
        if ($completedtmp) {
            $responses = $DB->get_records('evaluation_valuetmp', ['completed' => $completedtmp->id]);
        }
        return $responses;
    }
    ...
    public function get_finished_responses() {
        ...
        if ($this->completed) {
            $responses = $DB->get_records('evaluation_value', ['completed' => $this->completed->id]);
        }
        return $responses;
    }
    ...
    protected function create_current_completed_tmp() {
		...
		if ($this->get_courseid()) {
			...
			$record->course_of_studies = evaluation_get_course_of_studies( $this->get_courseid(), false );
        }
		if ( $this->get_teacherid() )
		{	$record->teacherid = $this->get_teacherid(); }
		elseif ( isset($_SESSION['teacherid']) AND $_SESSION['teacherid'] )
		{	$record->teacherid = $_SESSION['teacherid']; }
		/*if ( $this->get_course_of_studies() )
		{	$record->course_of_studies = $this->get_course_of_studies(); }
		elseif ( isset($_SESSION['course_of_studies']) AND $_SESSION['course_of_studies'] )
		{	$record->course_of_studies = $_SESSION['course_of_studies']; }*/
        if ((isloggedin() || $USER->id != $this->userid) && !isguestuser($this->userid)) {
        ...
        $id = $DB->insert_record('evaluation_completedtmp', $record);
        $this->completedtmp = $DB->get_record('evaluation_completedtmp', array('id' => $id) );
        $this->valuestmp = null;
        return $this->completedtmp;
    }
     ...
     *
     * @param stdClass $data data from the form mod_evaluation_complete_form
     */
    public function save_response_tmp($data) {
        ...
        foreach ($allitems as $item) {
            ...
            $newvalue = ['item' => $item->id, 'completed' => $completedtmp->id, 'courseid' => $completedtmp->courseid, 
			'teacherid' => $completedtmp->teacherid, 'course_of_studies' => $completedtmp->course_of_studies];
            ...
            if (array_key_exists($item->id, $existingvalues)) {
                ...
                $DB->update_record('evaluation_valuetmp', $newvalue);
            } else {
                $DB->insert_record('evaluation_valuetmp', $newvalue);
            }
        }
    ...
    public function save_response() {
        ...
        $this->completed = $DB->get_record('evaluation_completed', $params);

        // Send email.
        if ($this->evaluation->anonymous == EVALUATION_ANONYMOUS_NO) {
            evaluation_send_email($this->cm, $this->evaluation, $this->cm->get_course(), $this->userid, $this->completed);
        } else {
            evaluation_send_email_anonym($this->cm, $this->evaluation, $this->cm->get_course());
        }
        ...
        if ((isloggedin() || $USER->id != $this->userid) && $completion->is_enabled($this->cm) &&
                $this->cm->completion == COMPLETION_TRACKING_AUTOMATIC && $this->evaluation->completionsubmit) {
    ...
    protected function delete_completedtmp() {
        ...
        if ($completedtmp = $this->get_current_completed_tmp()) {
            $DB->delete_records('evaluation_valuetmp', ['completed' => $completedtmp->id]);
            $DB->delete_records('evaluation_completedtmp', ['id' => $completedtmp->id]);
    ...
    /**
     * Checks if user has capability to submit the evaluation
     *
    ...
    public function can_complete() {
        ...
        if (has_capability('mod/evaluation:complete', $context, $this->userid)) {
            return true;
        }
    ...
    public function can_submit() {
        if ($this->get_evaluation()->multiple_submit == 0 ) {
    ...
    public function trigger_module_viewed() {
        $event = \mod_evaluation\event\course_module_viewed::create_from_record($this->evaluation, $this->cm, $this->cm->get_course());
        $event->trigger();
    }
    ...
    /**
     * Process a page jump via the mod_evaluation_complete_form.
     *
    ...
    public function process_page($gopage, $gopreviouspage = false) {
        ...
        $this->form = new mod_evaluation_complete_form(mod_evaluation_complete_form::MODE_COMPLETE,
            $this, 'evaluation_complete_form', array('gopage' => $gopage));

        if ($this->form->is_cancelled()) 
		{	// Form was cancelled - return to the course page.
			$urltogo = "/mod/evaluation/view.php?id=".$this->cm->id; 
        } 
		elseif ($this->form->is_submitted() && ($this->form->is_validated() || $gopreviouspage)) 
			...
			if (!empty($data->savevalues) || !empty($data->gonextpage)) {
				if (($nextpage = $this->get_next_page($gopage)) !== null) {
					...
					if (!$this->get_evaluation()->page_after_submit) {
						\core\notification::success(get_string('entries_saved', 'evaluation'));
					}
