 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evaluation_course_select_form extends moodleform {
    /** @var moodle_url */
    protected $action;
    ...
    public function __construct($action, mod_evaluation_structure $evaluationstructure, $editable = true) {
        ...
        parent::__construct($action, null, 'post', '', ['id' => 'evaluation_course_filter'], $editable);
    }
    ...
    public function definition() {
        ...
        if (!$this->_form->_freezeAll && ($courses = $evaluationstructure->get_completed_courses()) && count($courses)) {
            ...
            if (0 and defined('BEHAT_SITE_RUNNING')) {
                ...
                $mform->addGroup($elements, 'coursefilter', get_string('filter_by_course', 'evaluation'), array(' '), false);
            }
        }
        $this->set_data(['courseid' => $evaluationstructure->get_courseid(), 'id' => $evaluationstructure->get_cm()->id]);
    }
}
