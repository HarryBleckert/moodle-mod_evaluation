...
if ($userid || $showcompleted) {
	...
	{	$eCourse = get_course( $completedrecord->courseid );
		...
		{	echo get_string('course_of_studies','evaluation') . ': <span style="font-size:12pt;font-weight:bold;display:inline;color:darkgreen;">'
	...
	{	$user = core_user::get_user( $completedrecord->teacherid );
		echo "Evaluation für " . get_string('teacher','evaluation') . 
			': <span style="font-size:12pt;font-weight:bold;display:inline;color:darkgreen;">' . $user->firstname. ' ' .$user->lastname."</span><br>\n"; 
	}
...
else 
{
	...
	{	
		...
		if ( !$courseid AND $SiteEvaluation )
...
{	if ( $completed_responses < $minresults )
	...
	if ( defined( "SiteEvaluation") AND 
		 ( (!$totalrowsRT AND !$totalrowsART) OR ($totalrowsRT < $minResults AND $totalrowsART < evaluation_min_results($evaluation)) ) )
	{	echo "<p style=\"color:red;font-weight:bold;align:center;\">".get_string('min_results', 'evaluation',$minResults)."</p>"; 
		evaluation_spinnerJS(false);
