 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_evaluation extends behat_base {

    /**
     * Adds a question to the existing evaluation with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the evaluation with:$/
     * @param string $questiontype
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_evaluation_with($questiontype, TableNode $questiondata) {

        $questiontype = $this->escape($questiontype);
        $additem = $this->escape(get_string('add_item', 'evaluation'));
        ...
        $saveitem = $this->escape(get_string('save_item', 'evaluation'));
     ...
     * The form for creating a question should be on one page.
     *
     * @When /^I add a page break to the evaluation$/
     */
    public function i_add_a_page_break_to_the_evaluation() {

        $questiontype = $this->escape(get_string('add_pagebreak', 'evaluation'));
        $additem = $this->escape(get_string('add_item', 'evaluation'));
     ...
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_log_in_as_and_complete_evaluation_in_course($username, $evaluationname, $coursename, TableNode $answers) {
        $username = $this->escape($username);
        $coursename = $this->escape($coursename);
        $evaluationname = $this->escape($evaluationname);
        $completeform = $this->escape(get_string('complete_the_form', 'evaluation'));

        // Log in as user.
        $this->execute('behat_auth::i_log_in_as', $username);

        // Navigate to evaluation complete form.
        $this->execute('behat_navigation::i_am_on_course_homepage', $coursename);
        $this->execute('behat_general::click_link', $evaluationname);
     ...
     */
    public function following_should_export_evaluation_identical_to($link, $filename) {
        global $CFG;
    ...
    /**
     * Ensures two evaluation export files are identical
     *
    ...
    protected function compare_exports($expected, $actual) {
        ...
        $dataexpected = $dataexpected['EVALUATION']['#']['ITEMS'][0]['#']['ITEM'];
        $dataactual = xmlize($actual, 1, 'UTF-8');
        $dataactual = $dataactual['EVALUATION']['#']['ITEMS'][0]['#']['ITEM'];
