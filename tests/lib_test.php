...
/**
 * Unit tests for (some of) mod/evaluation/lib.php.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evaluation_lib_testcase extends advanced_testcase {

    public function test_evaluation_initialise() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $pseudocm = get_coursemodule_from_instance('evaluation', $evaluation->id); // Object similar to cm_info.
        ...
        foreach ($constructorparams as $params) {
            $structure = new mod_evaluation_completion($params[0], $params[1], 0);
            $this->assertTrue($structure->is_open());
            $this->assertTrue($structure->get_cm() instanceof cm_info);
            $this->assertEquals($evaluation->cmid, $structure->get_cm()->id);
            $this->assertEquals($evaluation->intro, $structure->get_evaluation()->intro);
        }
    }

    /**
     * Tests for mod_evaluation_refresh_events.
     */
    public function test_evaluation_refresh_events() {
        global $DB;
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $course = $this->getDataGenerator()->create_course();
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_evaluation');
        $params['course'] = $course->id;
        $params['timeopen'] = $timeopen;
        $params['timeclose'] = $timeclose;
        $evaluation = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('evaluation', $evaluation->id);
        $context = context_module::instance($cm->id);

        // Normal case, with existing course.
        $this->assertTrue(evaluation_refresh_events($course->id));
        $eventparams = array('modulename' => 'evaluation', 'instance' => $evaluation->id, 'eventtype' => 'open');
        $openevent = $DB->get_record('event', $eventparams, '*', MUST_EXIST);
        $this->assertEquals($openevent->timestart, $timeopen);

        $eventparams = array('modulename' => 'evaluation', 'instance' => $evaluation->id, 'eventtype' => 'close');
        ...
        $this->assertTrue(evaluation_refresh_events());
        $eventparams = array('modulename' => 'evaluation');
        $events = $DB->get_records('event', $eventparams);
        foreach ($events as $event) {
            if ($event->modulename === 'evaluation' && $event->instance === $evaluation->id && $event->eventtype === 'open') {
                $this->assertEquals($event->timestart, $timeopen);
            }
            if ($event->modulename === 'evaluation' && $event->instance === $evaluation->id && $event->eventtype === 'close') {
     ...
     */
    public function test_evaluation_core_calendar_provide_event_action_open() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $actionevent = mod_evaluation_core_calendar_provide_event_action($event, $factory);

        $this->assertInstanceOf('\core_calendar\local\event\value_objects\action', $actionevent);
        $this->assertEquals(get_string('answerquestions', 'evaluation'), $actionevent->get_name());
    ...
    public function test_evaluation_core_calendar_provide_event_action_open_for_user() {
        ...
        $actionevent = mod_evaluation_core_calendar_provide_event_action($event, $factory, $user2->id);
        $this->assertNull($actionevent);

        // User2 checking $user's events.
        $actionevent = mod_evaluation_core_calendar_provide_event_action($event, $factory, $user->id);
        $this->assertInstanceOf('\core_calendar\local\event\value_objects\action', $actionevent);
        $this->assertEquals(get_string('answerquestions', 'evaluation'), $actionevent->get_name());
        $this->assertInstanceOf('moodle_url', $actionevent->get_url());
        $this->assertEquals(1, $actionevent->get_item_count());
     ...
     */
    public function test_evaluation_core_calendar_provide_event_action_closed() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        // No event on the dashboard if evaluation is closed.
        $this->assertNull($actionevent);
    }
     ...
     */
    public function test_evaluation_core_calendar_provide_event_action_closed_for_user() {
        global $DB;
     ...
     */
    public function test_evaluation_core_calendar_provide_event_action_open_in_future() {
        $this->resetAfterTest();
        $this->setAdminUser();
     ...
     * @throws coding_exception
     */
    public function test_evaluation_core_calendar_provide_event_action_open_in_future_for_user() {
        global $DB;
     ...
     * @throws coding_exception
     */
    public function test_evaluation_core_calendar_provide_event_action_no_time_specified() {
        $this->resetAfterTest();
        $this->setAdminUser();
     ...
     * @throws coding_exception
     */
    public function test_evaluation_core_calendar_provide_event_action_no_time_specified_for_user() {
        global $DB;
    ...
    /**
     * A user that can not submit evaluation should not have an action.
     */
    public function test_evaluation_core_calendar_provide_event_action_can_not_submit() {
        global $DB;
        ...
        $this->setUser($user);
        assign_capability('mod/evaluation:complete', CAP_PROHIBIT, $studentrole->id, $context);

        $factory = new \core_calendar\action_factory();
        $action = mod_evaluation_core_calendar_provide_event_action($event, $factory);

        $this->assertNull($action);
    }

    /**
     * A user that can not submit evaluation should not have an action, viewed by a different user.
     */
    public function test_evaluation_core_calendar_provide_event_action_can_not_submit_for_user() {
        global $DB;
    ...
    /**
     * A user that has already submitted evaluation should not have an action.
     */
    public function test_evaluation_core_calendar_provide_event_action_already_submitted() {
        global $DB;
    ...
    /**
     * A user that has already submitted evaluation should not have an action, viewed by a different user.
     */
    public function test_evaluation_core_calendar_provide_event_action_already_submitted_for_user() {
        global $DB;
    ...
    public function test_evaluation_core_calendar_provide_event_action_already_completed() {
        $this->resetAfterTest();
    ...
    public function test_evaluation_core_calendar_provide_event_action_already_completed_for_user() {
        $this->resetAfterTest();
        set_config('enablecompletion', 1);
        $this->setAdminUser();
        ...
        $actionevent = mod_evaluation_core_calendar_provide_event_action($event, $factory, $student->id);
     ...
     */
    public function test_mod_evaluation_completion_get_active_rule_descriptions() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $activeruledescriptions = [get_string('completionsubmit', 'evaluation')];
        $this->assertEquals(mod_evaluation_get_completion_active_rule_descriptions($cm1), $activeruledescriptions);
        $this->assertEquals(mod_evaluation_get_completion_active_rule_descriptions($cm2), []);
        $this->assertEquals(mod_evaluation_get_completion_active_rule_descriptions($moddefaults), $activeruledescriptions);
        $this->assertEquals(mod_evaluation_get_completion_active_rule_descriptions(new stdClass()), []);
    }
    ...
    public function test_get_valid_event_timestart_range_unknown_event() {
        ...
        $evaluation = $evaluationgenerator->create_instance(['course' => $course->id]);
        $evaluation->timeopen = $timeopen;
        $evaluation->timeclose = $timeclose;
    ...
    /**
     * A EVALUATION_EVENT_TYPE_OPEN should have a max timestart equal to the activity
     * close time.
     */
    ...
    /**
     * A EVALUATION_EVENT_TYPE_OPEN should not have a max timestamp if the activity
    ...
    /**
     * A EVALUATION_EVENT_TYPE_CLOSE should have a min timestart equal to the activity
     * open time.
     */
    ...
    /**
     * A EVALUATION_EVENT_TYPE_CLOSE should not have a minimum timestamp if the activity
    ...
    /**
     * An unkown event type should not change the evaluation instance.
     */
    public function test_mod_evaluation_core_calendar_event_timestart_updated_unknown_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
        ...
        $evaluation = $DB->get_record('evaluation', ['id' => $evaluation->id]);
        $this->assertEquals($timeopen, $evaluation->timeopen);
        $this->assertEquals($timeclose, $evaluation->timeclose);
    }

    /**
     * A EVALUATION_EVENT_TYPE_OPEN event should update the timeopen property of
     * the evaluation activity.
     */
    public function test_mod_evaluation_core_calendar_event_timestart_updated_open_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
        ...
        $evaluation->timeopen = $timeopen;
        $evaluation->timeclose = $timeclose;
        $evaluation->timemodified = $timemodified;
        $DB->update_record('evaluation', $evaluation);
        ...
        $this->assertEquals($newtimeopen, $evaluation->timeopen);
        // Ensure the timeclose isn't changed.
        $this->assertEquals($timeclose, $evaluation->timeclose);
        // Ensure the timemodified property has been changed.
        $this->assertNotEquals($timemodified, $evaluation->timemodified);
    }

    /**
     * A EVALUATION_EVENT_TYPE_CLOSE event should update the timeclose property of
     * the evaluation activity.
     */
    public function test_mod_evaluation_core_calendar_event_timestart_updated_close_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
        ...
        $this->assertEquals($newtimeclose, $evaluation->timeclose);
    ...
    public function test_student_role_cant_update_time_close_event() {
        ...
        $newevaluation = $DB->get_record('evaluation', ['id' => $evaluation->id]);
        // The activity shouldn't have been updated because the user
        // doesn't have permissions to do it.
        $this->assertEquals($timeclose, $newevaluation->timeclose);
    }
    ...
    public function test_teacher_role_can_update_time_close_event() {
        ...
        $this->assertEquals($newtimeclose, $newevaluation->timeclose);
    ...
    /**
     * A user who does not have capabilities to add events to the calendar should be able to create an evaluation.
     */
    public function test_creation_with_no_calendar_capabilities() {
        ...
        $generator = self::getDataGenerator()->get_plugin_generator('mod_evaluation');
