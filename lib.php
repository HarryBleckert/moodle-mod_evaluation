	...
	{	// create item Studiengang
		...
		{	if ( is_siteadmin() ){
			...
			{	list(	$_SESSION["participating_courses"], $_SESSION["participating_empty_courses"],
						$_SESSION["distinct_s"], $_SESSION["distinct_s_active"], $_SESSION["students"], $_SESSION["students_active"],  
			...
			// only up to 15 days
			if ( $forceGlobal AND (!empty($evaluation->participating_active_students ) AND ($timecloseSaved+(15*86400)) < time()) )
			{	
		...
		if ( $forceCourse OR !$DB->count_records_sql("SELECT COUNT(*) from {evaluation_enrolments} WHERE evaluation=$evaluation->id") )
		{	// set evaluation to Open for allowing set_results
			$evaluation->timeclose = time() + 86400;
			...
			{	$numTeachersCourse = $numStudentsCourse = $numTeachersActiveCourse = $numStudentsActiveCourse = 0;
				...
				{	$teacherids[] = $teacher['id']; $numTeachersCourse++;
					if ( $teacher["lastaccess"] > $timeopen)
					{	$active_teachers++; $numTeachersActiveCourse++; $active_teacherids[] = $teacher['id']; }
					//evaluation_user_lastaccess($evaluation, $teacher["id"], $teacher["lastaccess"], "teacher");
				}
				$teacherids = implode( ",", $teacherids );
				$active_teacherids = implode( ",", $active_teacherids );
				$fullname = $shortname = "''";
				if ( $course = $DB->get_record('course', array('id' => $courseid), '*') )
				{	$fullname = trim($course->fullname); $shortname = trim($course->shortname); }
				foreach ( $students AS $student )
				{	if ( $student["lastaccess"] > $timeopen )
					{	$active_students++; $numStudentsActiveCourse++; } 
					//evaluation_user_lastaccess($evaluation, $student["id"], $student["lastaccess"], "student");
				}
				$fields = array( "evaluation", "courseid", "fullname","shortname","course_of_studies", "students", "active_students", 
									"teacherids", "active_teachers", "active_teacherids", "timemodified");
				$values = array( $evaluation->id, $courseid, $fullname, $shortname,	$course_of_studies, safeCount($students), $numStudentsActiveCourse,
				...
				$recObj2 = $DB->get_record_sql( "SELECT id from {evaluation_enrolments} WHERE evaluation=$evaluation->id AND courseid=$courseid" );
		...
		if ( $forceUsers OR empty($evaluation->possible_evaluations) )
			/*$evaluation->timeclose > $DB->get_record_sql("SELECT timemodified from {evaluation_users} 
		...
		{	if ( true ) //evaluation_debug() ) 
			...
			{	$cnt = 1;
				...
				{	$userid = $complete->partid;
					...
					if ( $user )
					{	$username = trim($user->username); $firstname = trim($user->firstname); $lastname = trim($user->lastname); 
						$alternatename = trim($user->alternatename); $email = trim($user->email); 
					...
					// $lastaccess = evaluation_user_lastaccess($evaluation, $userid, $lastaccess, $participant);
					...
					$fields = array( "userid", "username", "firstname", "lastname", "alternatename", "email", "$utype", "lastaccess", "timemodified");
					$values = array( "$userid", "$username", "$firstname", "$lastname", "$alternatename", "$email", "$participated", $lastaccess, time() );
					$recObj = new stdClass();
					foreach ($fields AS $key => $value) 
					{	$recObj->{$value} = $values[$key]; }
					...
					if ( true ) //evaluation_debug() ) 
					{	print "<br>\n" .str_pad(number_format($cnt),6," ",STR_PAD_LEFT) . ". <b>$participant</b>: ".str_pad($userid,6) . " - " 
...
{	global $DB;
	...
	elseif ( $evaluation->anonymized AND !$force )
	...
	@ob_flush();@ob_end_flush();@flush();@ob_start();
	foreach ( $courses as $courseid )
	{	print '<script>document.getElementById("showCourseRec_' .$evaluation->id. '").innerHTML = "<b>' .$courseid. '</b> ('.$cntC.' courses)";</script>';
		$cntC++;
		@ob_flush();@ob_end_flush();@flush();@ob_start();
...
{   global $DB, $USER, $CFG;
    ...
    {   if ( $setD AND !defined( "SiteEvaluation") ) { define( "SiteEvaluation", true ); }
		$SiteEvaluation = true;
	}
	else
	...
	{	
		...
		{	if ( empty($_SESSION["LoggedInAs"]) )
			{	$CourseTitle = get_string("course","evaluation") . ": <span style=\"font-size:12pt;font-weight:bold;\"><a href=\"/course/view.php?id="
			...
			if ( $evaluation_is_open )
			{	//(3,4,5,9,12);" ); // get all teachers and students
				$roleT = $DB->get_records_sql( "SELECT * FROM {role} WHERE id IN (3,4,12,5);" ); // get only editingteachers, teachers and students 
			...
			if ( $isTeacher )
			{	$isPermitted = true;
				if ( $setD AND !defined( "isTeacher") ) { define( "isTeacher", true ); }
			}
	...
	{	$isPermitted = true; //( empty($_SESSION['CoS_privileged'][$user->username]) ?true :!$courseid ); 
		if ( $setD AND !defined( "EVALUATION_OWNER") ) { define( "EVALUATION_OWNER", $username ); } 
	}
    /*
	}
...
{	global $USER;
	...
	{	$_SESSION['CoS_privileged'] = array(); get_evaluation_filters( $evaluation ); }
...
{	global $USER;
	...
	elseif ( evaluation_is_WM_disabled($evaluation) )
	{	$excluded = array();
		foreach ( $_SESSION["course_of_studies_wm"] AS $CoS => $is_WM )
		{	if ( $is_WM )	{	$excluded[] = $CoS; } }
		if ( !empty($excluded) )
		{	$filter = " AND " .($tableName? $tableName."." :""). "course_of_studies ";	
...
{	global $USER;
	...
	{	$sg_filter = explode( "\n", $evaluation->filter_course_of_studies); 
		$excluded = array();
		foreach ( $_SESSION["course_of_studies_wm"] AS $CoS => $is_WM )
		{	if ( $is_WM AND in_array($CoS, $sg_filter) )	{	$excluded[] = $CoS; } } 
...
{   global $DB;
	...
	if ( false ) //evaluation_debug( false ) )
	{	print "<br><hr><b>CoS</b>: ". nl2br(var_export($CoS,true))."<hr>Courses:<br>".nl2br(var_export($courses,true))."<hr>\n"; }
	return $CoS;
}
...
{	global $DB;
	$filter = evaluation_get_cosPrivileged_filter( $evaluation );
    $count = $DB->count_records_sql( "SELECT COUNT(*) FROM {evaluation_completed} WHERE evaluation=$evaluation->id AND courseid=$courseid $filter" );
	if ( false AND evaluation_debug( false ) )
...
{	global $DB, $USER;
	...
	//$DB->set_debug(true);
	$filter = evaluation_get_cosPrivileged_filter( $evaluation );
	$sql = "SELECT COUNT(*) FROM {evaluation_completed} WHERE evaluation=$evaluation->id AND ( userid=$userid OR teacherid=$userid ) $filter";
...
{	global $CFG, $DB, $USER, $PAGE, $OUTPUT, $id, $teacheridSaved, $courseid, $teacherid, $course_of_studiesID ,$evaluation, $downloading;
	...
	if ( !defined( "SiteEvaluation") OR !empty($downloading) 
			OR ( isset($_SESSION["EVALUATION_OWNER"]) AND $_SESSION["EVALUATION_OWNER"] !== $USER->id AND empty($_SESSION["LoggedInAs"]) AND empty($USER->realuser) ) )	
	{	return false; }	
	$userid = false;
	...
	elseif ( stristr( $role, "privileg") )
	{	$role = "Privilegierte Person";
		if ( !empty($evaluation->privileged_users) OR !empty($_SESSION["privileged_global_users"]) )
		{	$privileged_users = explode("\n", $evaluation->privileged_users);
			if ( !empty($_SESSION["privileged_global_users"]) )
			{	//array_merge_recursive_distinct( $privileged_users, $_SESSION["privileged_global_users"]);
	...
	*/	
	...
	{	$role = (stristr( $_SESSION["LoggedInAs"], "privileg") ?"privilegierte Person" 
				:(stristr( $_SESSION["LoggedInAs"], "dekan") ?"Dekan_in" :$DB->get_record('role', array('shortname'=>$_SESSION["LoggedInAs"]), '*')->name ));
...
{	global $DB;
	$userid = 38333;
	$users = $DB->get_records_sql("SELECT id from {user} WHERE deleted=0 AND suspended=0 AND lastaccess>0 ORDER by lastaccess ASC LIMIT 1000");
...
{	global $DB;
	...
	{	$students = get_evaluation_participants($evaluation, false, $courseid, false, true);
		foreach ( $students AS $student )
		{	if ( $student->lastaccess > $timeclose )	{	$active_students++; } }
...
{	global $DB;
	...
	if ( empty($evaluation->possible_evaluations) ) {
		...
		foreach ( $enrolments AS $enrolment){
			...
			{	$_SESSION["possible_evaluations"][$enrolment->courseid] = $possible_evaluations += ($enrolment->students * safeCount(explode(",",$enrolment->teacherids))); }
			if ( $enrolment->active_students AND !empty($enrolment->active_teachers) )
			{	$_SESSION["possible_active_evaluations"][$enrolment->courseid] = $possible_active_evaluations += $enrolment->active_students * $enrolment->active_teachers; }
		}
	}
...
{	global $DB, $evaluation;
	...
	{	$my_evaluation_users = array();
		if ( empty($_SESSION["allteachers"]) ) { $_SESSION["allteachers"] = array(); }
		$evaluation_is_open = true;
		...
		{	$course = $DB->get_record('course', array('id' => $courseid), '*'); 
			...
			{	$_SESSION["allteachers"][$courseid] = $my_evaluation_users; return; }
			$contextC = context_course::instance($courseid);
			if ( is_array( $contextC ) OR is_object( $contextC ) )
			{	$roleT = $DB->get_records_sql( "SELECT * FROM {role} WHERE id IN (3,4,12);" ); // get only editingteachers //(3,4,5,9,12)
				foreach ( $roleT as $role )
				{	$rolesC = get_role_users($role->id, $contextC);
					foreach ($rolesC as $roleC )
					{ 	//echo "<hr>$roleC->lastname\n" . var_export($roleC,true)."<br>\n"; 
		...
		{	$CourseRec = $DB->get_record_sql( "SELECT DISTINCT ON (courseid) courseid, fullname, shortname, teacherids, id 
			...
			{	$teacherids = explode(",", $CourseRec->teacherids);
				...
				{	if ( !empty( $teacherid) AND !isset($my_evaluation_users[$teacherid]) ) 
					{	$userRec = $DB->get_record_sql( "SELECT DISTINCT ON (userid) * FROM {evaluation_users} WHERE userid = $teacherid"); 
						if ( isset( $userRec->userid ) )
						{	$fullname = ($userRec->alternatename ?$userRec->alternatename :$userRec->firstname). " " . $userRec->lastname;
		...
		uasort($my_evaluation_users, function($a, $b) {	return strcmp($a['lastname'], $b['lastname']);} );
...
{	global $USER;
	...
	if ( empty($cmid) )	{ $cmid = get_evaluation_cmid_from_id($evaluation); }
	...
	{	$showTeachers = "<br>Dozent_in".(safeCount($teachers)>1 ?"nen" :"").": ";
		...
		{	if ( defined( "EVALUATION_OWNER") OR $teacher["id"] == $USER->id )
			{	$showTeachers .= '<a href="/mod/evaluation/print.php?id='.$cmid.'&showTeacher='.$teacher["id"].'&courseid='.$courseid.'">'; }
			elseif ( !empty($_SESSION["LoggedInAs"]) )
			{	$showTeachers .= '<a href="#">'; }
			else
...
function evPrintButton()
	{	$buttonStyle = 'margin: 3px 5px;font-weight:bold;background-color:white;';
		...
		print html_writer::tag( 'a', '<img class="image" alt="Auswertung Drucken" title="Auswertung Drucken" 
			...
			array( 'id' => 'printPage', 'style' =>$buttonStyle, 'href' => 'javascript: window.print();','title'=>'Auswertung Drucken'));
		echo '</div>';
	}


// show loading spinner icon / font awesome required
function evaluation_showLoading()
...
{	if ( !$hide )
	{	/*print '<script>document.getElementById("spinner").style.display="none";
...
{	if ( !isset($_SESSION[$var]) ) { $_SESSION[$var] = $preset; }
...
{	$timeopen = ($evaluation->timeopen>0 ?$evaluation->timeopen :0);
	...
	{	//return round(($difference/60/60/24)+1,1);
...
{	global $DB;
	...
	/*if ( $is_closed ) {
		...
		if ( empty($cat->path) )
		{	return array(); }
		$course_studies_raw = $DB->get_records_sql( "select id,idnumber,name AS course_of_studies from {course_categories} where path like '".$cat->path."/%'
	...
	if ( $raw )
	{	foreach ( $sgNtmp as $studyname)	{	$studynames[] = $studyname; }
		return $studynames; 
	}
	...
	{	if ( !$raw AND 
				( $studiengang->course_of_studies == "weitere Veranstaltungen" OR $studiengang->course_of_studies == "Zusatzveranstaltungen" 
		...
		if ( $link )
		{	$studiengang->course_of_studies = '<a href="/course/index.php?categoryid='.$studiengang->id.'" target="studiengang">'.$studiengang->course_of_studies."</a>\n"; }
...
{	global $DB;
	...
	{	$studiengang = $DB->get_record_sql("SELECT id, course_of_studies AS name FROM {evaluation_completed} WHERE courseid = $courseid LIMIT 1"); 
	...
	{	$course = $cat = array();
		...
		{	if ( !isset($path[COURSE_OF_STUDIES_PATH +1 ]) )
			...
			{	return '<a href="/course/index.php?categoryid='.$semester->id.'" target="semester">'.$semester->name."</a>\n"; }
	...
	if ( isset($studiengang->name) AND !empty($studiengang->name) )
	{	//set new value to course->customfield_studiengang, disabled because customfield was removed
		if ( $link AND !empty($studiengang->id) AND empty($_SESSION["LoggedInAs"]) )
		{	return '<a href="/course/index.php?categoryid='.$studiengang->id.'" target="studiengang">'.$studiengang->name."</a>\n"; }
...
{	global $DB;
	...
	foreach ( $studytmp as $studyname)	{	$studynames[] = $studyname; }
...
function get_courseid_from_course_of_studies( $evaluation, $course_of_studies )
// unused 20221212
{	global $DB;
...
{	global $DB;
	...
	{	//$DB->set_debug(true);
		...
		//$DB->set_debug(false);				  
		foreach ( $myCourses AS $course ) { $ids[$course->courseid] = $course->courseid; }	
		//if ( safeCount($ids) >0	) { $fcourses = "AND ".($evaluation_is_open?"" :"course")."id IN (".implode(",",$ids).")"; }
		if ( safeCount($ids) >0	) { $fcourses = "AND ".((true OR $evaluation_is_open)?"" :"course")."id IN (".implode(",",$ids).")"; }
		else
		{ return $ids; }
	}
	$myCourses = $DB->get_records_sql("SELECT id FROM {course} WHERE visible=1 AND RIGHT(idnumber,5) = '$evaluation_semester' $fcourses" ); 
	if ( !$evaluation_is_open )
...
function evaluation_user_lastaccess($evaluation, $userid, $lastaccess=0, $role="student")
{	global $DB;
	if (empty($lastaccess))
	{	$lastaccess = 0; }
	if (!isset($evaluation->timeclose))
	{	return 0; } //$lastaccess; }
	$userlast = $DB->get_record_sql("SELECT * from {evaluation_users_la} WHERE evaluation=".$evaluation->id." AND userid=$userid LIMIT 1");
	$is_open = evaluation_is_open($evaluation);
	if ( isset($userlast->lastaccess) AND !$is_open )
	{	return ($userlast->lastaccess ?:0); }
	elseif ( $is_open )
	{	if ( !isset($userlast->lastaccess) ){
			$fields = array( "evaluation", "userid", "role", "lastaccess", "timemodified");
			$values = array( $evaluation->id, $userid, $role, $lastaccess, time() );
			$recObj = new stdClass();
			foreach ($fields AS $key => $value) 
			{	$recObj->{$value} = $values[$key]; }
			$DB->insert_record('evaluation_users_la', $recObj);
		}
		elseif ( $lastaccess > $userlast->lastaccess )
...
{	global $USER;
	...
	{	if ( $myEvaluation['role'] == "student" AND $myEvaluation['id'] == $USER->id ) 
		{	if ( !$courseid ) { return true; }
			if ( $myEvaluation['courseid'] == $courseid ) { return true; }
		}
	}
	return false;
}
...
{	global $DB, $USER; 
	...
	{	$numTeachersCourse = $numStudentsCourse = $numTeachersActiveCourse = $numStudentsActiveCourse = 0;
		...
		$courseid = $course->id;
		// full filtering
		list( $show, $reminder) = evaluation_filter_Evaluation($course->id, $evaluation, $user ); //($userid AND $userid > 0 ?$user :false) );
		if ( !$show )
		{	continue; }	
		$cnt_courses++;
			...
			{	$rolesC = get_role_users($role->id, $contextC);
				...
				{ 	$fullname = ($roleC->alternatename ?$roleC->alternatename :$roleC->firstname). " " . $roleC->lastname;
					...
					{	$cnt_students++;
						...
						$roleC->lastaccess = evaluation_user_lastaccess($evaluation,$roleC->id, $roleC->lastaccess, "student");
						...
						{	$cnt_students_active++; $numStudentsActiveCourse++; $distinct_s_active[$roleC->id] = $fullname; }
						...
						{	$my_evaluation_courses[$course->id] = array( "role"=> "student", "id" => $roleC->id, 
										...
										"email" => $roleC->email, "fullname" => $fullname, "courseid" => $course->id, "course" => $course->fullname, "shortname" => $course->shortname, "lastaccess" => $roleC->lastaccess,
										"reminder" => $reminder ); 
						}
						elseif ( $getStudents )
						{	$my_evaluation_users[$roleC->id] = array( "fullname" => $fullname, "id" => $roleC->id, "username" => $roleC->username,
					...
					{	$cnt_teachers++;
						...
						$roleC->lastaccess = evaluation_user_lastaccess($evaluation,$roleC->id, $roleC->lastaccess, "teacher");
						// get inactive teachers
						//if ( $roleC->lastaccess > ( $evaluation_is_open ?(time()-(24*3600*$total_evaluation_days) ) :$timeopen ) ) 
						if ( $roleC->lastaccess > $timeopen ) // ( $evaluation_is_open ?$timeopen :$timeclose ) ) 
						//if ( $roleC->lastaccess > $timeopen )
						{	$cnt_teachers_active++; $numTeachersActiveCourse++; $distinct_t_active[$roleC->id] = $fullname; }
						if ( $userid AND ( $userid < 0 OR $userid == $roleC->id ) )
						{	$my_evaluation_courses[$course->id] = array( "role"=> "teacher", "id" => $roleC->id,  "username" => $roleC->username, 
										"email" => $roleC->email, "fullname" => $fullname, "courseid" => $course->id, "course" => $course->fullname,
		...
		{	// get students
			...
			{	$fullname = ($roleC->alternatename ?$roleC->alternatename :$roleC->firstname). " " . $roleC->lastname;
				...
				{	$cnt_teachers++; $numTeachersCourse++; }
				$distinct_t[$roleC->id] = $fullname; //=$cnt_teac
				// get inactive teachers
				//$userdata = core_user::get_user($roleC->id);
				$roleC->lastaccess = evaluation_user_lastaccess($evaluation,$roleC->id, $roleC->lastaccess, "teacher");
				if ( $roleC->lastaccess > $timeopen AND !isset( $distinct_t_active[$roleC->id] ))
				{	$cnt_teachers_active++; $numTeachersActiveCourse++;
...
{	global $CFG, $DB, $USER;
	...
	{	
		...
		if ( $sortby == "course" )
		{	uasort($myEvaluations, function($a, $b) {	return strcoll(strtoupper($a["course"]), strtoupper($b["course"])); }); }
		else
		{	uasort($myEvaluations, function($a, $b) {	return strcoll(strtoupper($a["course"]), strtoupper($b["course"])); }); }
			...
			if ( $myEvaluation["role"] != "teacher" ){
				...
				if ( !$evaluation_is_open AND $num_courses > safeCount($myEvaluations)){
					...
					$str .= "\n<b>Hinweis</b>: Ihnen ".(safeCount($myEvaluations)>1 
						?"werden nur die ".safeCount($myEvaluations)." Kurse angezeigt, ".($isstudent ?"für die Sie" :"für die")
		...
		{	$evaluation_has_user_participated = true;
			...
			if ( !$isTeacher ) {
				$evaluation_has_user_participated = evaluation_has_user_participated($evaluation, $USER->id, $myEvaluation["courseid"] );
			}
			...
			{	$replies = ""; 
				...
				{	// get course teachers
					...
					{	$Txt = $actionTxt;
						...
						{	$color = "darkred";
							$Txt .= '<span style="font-weight:normal;color:black;"><br>Dozent_in '.$teacher['fullname'].'</span>';
							$url = "<a href=\"$wwwroot/mod/evaluation/complete.php?id=$cmid&courseid=".$myEvaluation["courseid"]."&teacherid=".$teacher['id'].'">'; 						
	...
	elseif ( !defined('EVALUATION_OWNER') AND $evaluation->timeclose<time() ) // ".get_string('students_only', 'evaluation')."
...
function showEvaluationCourseResults($evaluation, $showMin=3, $sortBy="fullname", $id = false, $courseid = false, $teacherid = false )
{	global $DB, $USER;
	if ( !$id )
	...
	{	$notevaluated = true;
		...
		$evaluated = $DB->get_records_sql("SELECT distinct courseid FROM mdl_evaluation_completed WHERE evaluation=$evaluation->id $cosPrivileged_filter");
...
//print "<br>Query: $query<br>";exit;
	...
	$goBack = html_writer::tag( 'a', "Zurück", array( 'class' =>"d-print-none", 'style'=>'font-size:125%;color:white;background-color:black;text-align:right;', 
	...
	<input type="number" name="showResults" value="<?php echo $showMin;?>" style="font-size:100%;color:white;background-color:teal;">
	...
	<br>
	<input type="submit" name="notevaluated" style="font-size:100%;color:white;background-color:black;" value="Kurse ohne Abgaben">
	</form>

	<form style="display:inline;line-break:inline;" method="POST" action="print.php">
	<button name="showCompare" style="font-size:100%;color:white;background-color:black;" value="1" onclick="this.form.submit();"><?php 
	...
	{	$params .= "&notevaluated=Kurse ohne Abgaben";
		...
		{	$Studiengang = evaluation_get_course_of_studies($result->id,true);  // get Studiengang with link
			...
			if ( !$numTeachers )
			{	$noteacher_courses++; $numTeachers = '<span style="color:red;font-weight:bold;">0</span>'; }
			if ( !$numStudents )
			{	$nostudent_courses++; $numStudents = '<span style="color:red;font-weight:bold;">0</span>'; }
			
			$output .= '<tr><td>'.$is_empty.'</td><td style="text-align:right;">'.$numTeachers.'</td><td style="text-align:right;">'
				.$numStudents.'</td><td>'.$courseL.'<span title="'.$result->fullname.'">'
				.substr($result->fullname,0,60).(strlen($result->fullname)>60 ?'...':'') .'</span></a></td><td>'
				.$result->shortname.'</td><td style="text-align:right;">'.$result->id."</td><td>" .$Studiengang. "</td></tr>\n";	
		}
		$output .=  "</table>\n";
		$show_stats = "";
		if ( $cosPrivileged_filter )
	...
	{	$sort_sym = ($_SESSION["orderBy"] == "ASC" ?"&uarr;" :"&darr;");
		...
		$header = '<tr style="font-weight:bolder;">
				<td style="text-align:right;"><a href="?id='.$params.'&sortBy=evaluations"><span style="color:teal;">'.$Abgaben.'</span></a></td>
				<td style="text-align:right;"><a href="?id='.$params.'&sortBy=numTeachers"><span style="color:teal;">'.$Doz.'</span></a></td>
				<td style="text-align:right;"><a href="?id='.$params.'&sortBy=numStudents"><span style="color:teal;">'.$Stud.'</span></a></td>
		...
		{	sort($evaluations);
			...
			$output .= '<tr><td colspan="4"><b>Abgaben aus diesen '.evaluation_number_format($sumC)
						.' Kursen:</b></td><td colspan="2" style="text-align:right;"><b>' .evaluation_number_format($sumR)."</b></td></tr>\n";
			$output .= '<tr><td colspan="4"><b>Abgaben/Kurs: Median:</b></td><td colspan="2" style="text-align:right;"><b>'.evaluation_number_format($median)."</b></td></tr>\n";
			$output .= '<tr><td colspan="4"><b>Abgaben/Kurs: Mittelwert:</b></td><td colspan="2" style="text-align:right;"><b>'.evaluation_number_format($average)."</b></td></tr>\n";
			$output .= '<tr><td colspan="4"><b>Abgaben/Kurs: Modus:</b></td><td colspan="2" style="text-align:right;"><b>'.evaluation_number_format($modus)."</b></td></tr>\n";
			if ( $sumC >= 12 ) {	$table .= $output; }
...
function showEvaluationTeacherResults($evaluation, $showMin=6, $sortBy="lastname", $id = false, $courseid = false, $teacherid = false )
{	global $DB, $USER;
	if ( !$id )
	{	$id = get_evaluation_cmid_from_id($evaluation); }
	...
	{	$notevaluated = true;
		...
		$evaluated = $DB->get_records_sql("SELECT distinct teacherid FROM mdl_evaluation_completed WHERE evaluation=$evaluation->id" );
										//. $cosPrivileged_filter);
		$evaluated_teachers = 0;
		foreach ( $evaluated AS $teacher )
		{	if ( isset( $allteachers[$teacher->teacherid] ) ) 
			{	$evaluated_teachers++; unset($allteachers[$teacher->teacherid]); }
		}
	...
	$return_to = (isset($_REQUEST["goBack"]) ? $_REQUEST["goBack"] :"analysis_course");
	$goBack = html_writer::tag( 'a', "Zurück", array( 'class' =>"d-print-none",'style'=>'font-size:125%;color:white;background-color:black;text-align:right;', 
								'type' => 'button','href' => $return_to.'.php?id='.$id.'&courseid='.$courseid.'&teacherid='.$courseid ));
	print $goBack;
	echo evPrintButton();
	...
	<input type="number" name="showTeacherResults" value="<?php echo $showMin;?>" style="font-size:100%;color:white;background-color:teal;">
	<input type="hidden" name="sortBy" value="<?php echo $sortBy;?>">
	<input type="hidden" name="courseid" value="<?php echo $courseid;?>">
	<input type="hidden" name="id" value="<? echo $id;?>">
	<input type="hidden" name="goBack" value="<? echo $return_to;?>">
	...
	{	$listed = 0;
		...
		{	/*if ( !evaluation_is_user_enrolled($evaluation,  $teacher['id'] ) )
			{ continue; }*/
			$show = !ev_is_user_in_CoS( $evaluation, $teacher['id'] );
			if ( $show ) 
			{	$listed ++;
				$showTeacher = '<a href="/mod/evaluation/print.php?id='.$id.'&showTeacher='.$teacher['id'].'" target="teacher">';
				print '<tr><td>'.$showTeacher . $teacher['firstname'].'</td><td>'.$showTeacher . $teacher['lastname'] . "</a></td></tr>\n";	
			}
		}
		print "</table>\n";
		print '<b>Anzahl aller ausgewerteten Dozent_innen mit Abgaben: '.evaluation_number_format($evaluated_teachers).'</b><br>'."\n";				
	...
	{	$sort_sym = ($_SESSION["orderBy"] == "ASC" ?"&uarr;" :"&darr;");
		...
		$header = '<tr style="font-weight:bolder;"><td style="text-align:right;"><a href="?id='
					...
					<td><a href="?id='.$params.'&sortBy=lastname"><span style="color:teal;">'.$Lastname.'</span></a></td></tr>';
		foreach ( $results as $result )
		{	if ( $result->evaluations >= $showMin )
			{	$sumC ++;
				...
				$showTeacher = '<a href="/mod/evaluation/print.php?id='.$id.'&showTeacher='.$result->id.'" target="teacher">'.$result->firstname.'</a>';
		...
		{	
			...
			$output .= '<tr><td colspan="2"><b>Anzahl der ausgewerteten Dozent_innen mit mindestens '.$showMin
						.' Abgaben:</b></td><td colspan="1" style="text-align:right;"><b>' .evaluation_number_format($sumC). "</b></td></tr>\n";
			...
			$output .= '<tr><td colspan="2"><b>Abgaben/Dozent_in: Median:</b></td><td colspan="1" style="text-align:right;"><b>'.evaluation_number_format($median)
						."</b></td></tr>\n";
			$output .= '<tr><td colspan="2"><b>Abgaben/Dozent_in: Mittelwert:</b></td><td colspan="1" style="text-align:right;"><b>'.evaluation_number_format($average)
						."</b></td></tr>\n";
			$output .= '<tr><td colspan="2"><b>Abgaben/Dozent_in: Modus:</b></td><td colspan="1" style="text-align:right;"><b>'.evaluation_number_format($modus)
						."</b></td></tr>\n";
			if ( $sumC >= 12 ) {	$table .= $output; }
...
{	global $DB;
	...
	$courses = evaluation_participating_courses($evaluation);
	$cnt = 0;
	$blockinstance = $DB->get_record_sql("SELECT id FROM {block_instances} where blockname = 'evaluation' and parentcontextid=1 LIMIT 1");
	$corrections = "";
	foreach ( $courses as $courseid )
	{	list( $show, $reminder) = evaluation_filter_Evaluation($courseid, $evaluation);
		...
		if ( $coursecontext AND isset($coursecontext->id ) AND isset($blockinstance->id) )
		{	// move blocks with position < -10 down to -9
			...
			{	$cnt++;
				$corrections .= str_pad($cnt,4, " ",STR_PAD_LEFT). " On top of Evaluation: " . safeCount($onTop) . ' Course: <a href="/course/view.php?id='.
...
{	global $DB;
	if ( !$courseid OR !$userid ) { return false; }
	$fteachers = ""; $ids = array(); 
	$teamteaching = $evaluation->teamteaching;
	$evaluationid = 0;
	evaluation_get_course_teachers( $courseid); 
...
{	if ( $evaluation->teamteaching_courses >0 AND $evaluation->timeclose < time() )
	{	$_SESSION["teamteaching_courses"] = $evaluation->teamteaching_courses; return $evaluation->teamteaching_courses; }
...
{	global $DB;
	...
	if ( !empty($courseid) )
	{	// if courseid is $myEvaluations
		if ( is_array($courseid) )
		{	$ids = array();
			foreach ( $courseid AS $myEvaluation)
			{	$ids[] = $myEvaluation["courseid"]; 
				if ( evaluation_debug( false ) )	{	print "<hr>myEvaluation: " .nl2br(var_export($myEvaluation)); }
			}			
			if ( safeCount($ids) )
...
{	return ( $evaluation->timeclose < time() ); }	// $evaluation->timeclose > 0 AND  
...
{	$passed = true;
	...
	{	$Studiengang = evaluation_get_course_of_studies($courseid); 
		...
		if ( $passed AND !empty($courses_filter) AND in_array( $courseid, $courses_filter )) { $passed = false; }
		//print "<br><br><hr>Evaluation: $evaluation->name - Studiengang: $Studiengang - ".var_export($sg_filter,true)." - showEval: ".($showEval ?"Yes" :"No") ."<br>\n";
	}
	elseif ( !empty($courses_filter) AND !in_array( $courseid, $courses_filter )) { $passed = false; }
...
{	global $CFG, $USER;
	...
	{	$cfgA = explode( "\n", file_get_contents( $cfgFile ) );
		$privileged_users = $_SESSION["privileged_global_users"] = $_SESSION["privileged_global_users_wm"] = $_SESSION["course_of_studies_wm"] = array();
		...
		{	
			...
			{	continue; }
			if ( isset($parts[1]) )	{	$CoS = trim($parts[1]); }
			if ( isset($parts[2]) )	{	$WM = trim($parts[2]); }
			...
			elseif 	( is_array($_SESSION['filter_course_of_studies']) AND !empty($_SESSION['filter_course_of_studies']) 
	...
	elseif (!isset($_SESSION['ev_global_cfgfile'])) {
...
{	global $CFG, $USER;
	...
	if ( isset( $_SESSION['filter_course_of_studies']) AND isset( $_SESSION['filter_courses']) AND isset($_SESSION['CoS_privileged']) )
...
{ 	global $DB, $USER;
	...
	if ( $no_user)	{	$isPermitted = true; }
	if ( !$SiteEvaluation)	{	return array(true,""); }
	...
	if ( ($SiteEvaluation AND trim(substr( $course->idnumber, -5)) != $evaluation_semester)
		OR ( intval(date("Ymd",$timeopen)) > intval(date("Ymd")) AND !$showPriv ) )
	...
	if (!$no_user AND $showEval AND !$isPermitted AND intval(date("Ymd",$timeopen)) <= intval(date("Ymd"))) {
		...
		{	if ( !isEvaluationCompleted( $evaluation, $courseid, $user->id ) )
			{	$has_user_participated = evaluation_has_user_participated($evaluation, $user->id );
				...
				if ( $days <7 )
				{	$style = '<b style="color:red;">Die Abgabefrist endet '.($days>0 ?"in ".$days.' Tagen' :"heute").'</b><br><b style="color:teal;"> '; }
		...
		elseif( !evaluation_has_user_participated($evaluation, $user->id, $courseid ) ) {
			$reminder = "";
		}
	}
...
function evaluation_calc_perc($number, $total) 
...
function evaluation_number_format($number, $decimals=0) 
{	// use Moodle lang setting for display options
    $dsep = "."; $tsep = ","; 
...
{	global $DB;
	...
	$completed = $DB->get_records_sql("SELECT DISTINCT courseid AS courseid FROM {evaluation_completed} WHERE courseid>0 AND course_of_studies=''".
				" ORDER BY courseid");
	foreach ( $completed AS $complete )
	...
	$values = $DB->get_records_sql("SELECT DISTINCT courseid AS courseid FROM {evaluation_value} WHERE courseid>0 AND course_of_studies IS NULL".
				" ORDER BY courseid");
	foreach ( $values AS $value )
	{ $updated = $DB->execute("UPDATE {evaluation_value} SET course_of_studies='"
				.evaluation_get_course_of_studies( $value->courseid)."' WHERE course_of_studies='' AND courseid=".$value->courseid);
	}
}
...
{	global $DB;
	...
	{ 	evaluation_get_course_teachers( $complete->courseid);
		$teacherid = 0;
		if ( empty($complete->teacherid) AND !empty($_SESSION["allteachers"][$complete->courseid]) AND safeCount($_SESSION["allteachers"][$complete->courseid]) == 1 )
		{	foreach ($_SESSION["allteachers"][$complete->courseid] AS $teacher) 
			{ $teacherid = $teacher['id']; break; } 
...
{	global $DB;
	...
	{ 	$counter++;
		...
		{	continue; }*/
		if ( !empty($_SESSION["allteachers"][$complete->courseid]) AND safeCount($_SESSION["allteachers"][$complete->courseid])>0 )
		{	$cnt = 0;
			foreach ( $_SESSION["allteachers"][$complete->courseid] AS $teacher ) 
...
if ( $counter >= 6)	{ exit;} */
				...
				print '<script>document.getElementById("counter").innerHTML = "'
					. "Count: $counter - ID: $complete->id: CourseID: $complete->courseid - UserID: $complete->userid - Teacherid: $teacherid".'";</script>';
				@ob_flush();@flush();
			}
		}
	}
...
{	if ( !is_siteadmin() AND !isset($_SESSION["LoggedInAs"]) )
	{	global $DB;
		if ( !$courseid ) {	$courseid = $evaluation->course; }
		if ( !$cm ) {	list($course, $cm) = get_course_and_cm_from_cmid($id, 'evaluation'); }
		else {	$course = $DB->get_record('course', array('id' => $courseid), '*'); }
...
{	global $DB;
	$is_item  = $DB->get_record_sql("SELECT id FROM {evaluation_item} WHERE evaluation = $evaluationid AND name = 'course_of_studies'");
...
{	
	...
	$courseids = $DB->get_records_sql("SELECT * FROM {evaluation_item} WHERE name ILIKE '%tudiengang%' AND evaluation=$evaluation->id and typ='multichoice'");
	...
	// create item Studiengang
	{	$item = $DB->get_record_sql("SELECT * FROM {evaluation_item} WHERE evaluation=$evaluation->id and typ='multichoice' LIMIT 1");
		if ( !isset($item->id)) 
		...
		$template = $item->id;
		unset($item->id);
		$item->name = "Studiengang"; $item->label = ""; $item->hasvalue = 1; $item->position = $position;
		$item->required = 0; $item->dependitem = 0; $item->options = "h";
	...
	$courseids = $DB->get_records_sql("SELECT * FROM {evaluation_value} WHERE item=$template AND courseid>0 ORDER BY completed ASC");
	$hits = safeCount($courseids);
	$cnt = 0;
	print '<br><span id="counter"></span><br>';
	foreach ( $courseids AS $courseid )
	{	// id	courseid	item	completed	value
		set_time_limit(30);
		$cnt++;
		$newval = new stdClass();
		...
		print '<script>document.getElementById("counter").innerHTML = "'."$cnt/$hits: $val_id - $courseid->courseid - $Studiengang - Wert:"
...
{	
	...
	*/
	...
	{	
		...
		$cm->module = $module->id; $cm->instance = $eID;
		$cmid = $cm->id;
		...
		$evaluations_completed = $DB->get_records_sql("SELECT * FROM {evaluation_completed} WHERE evaluation=$evaluationid ORDER BY id");
		...
		{	$evaluation_item->evaluation = $eID;
			...
			$values = safeCount($evaluation_values); $cnt = 0;
			foreach ( $evaluation_values as $evaluation_value )
			{	$cnt++;print '<script>document.getElementById("counter").innerHTML = "'.$cnt." of ".$values.' rows in values table";</script>';
 ...
 */
function evaluation_is_empty_course($courseid) 
{	global $DB;
...
function evaluation_update_instance($evaluation) {
    ...
    \core_completion\api::update_completion_date_event($evaluation->coursemodule, 'evaluation', $evaluation->id, $completionexpected);
...
function evaluation_reset_course_form($course) {
    ...
    echo get_string('resetting_evaluations', 'evaluation'); echo ':<br />';
    if (!$evaluations = $DB->get_records('evaluation', array('course'=>$course->id), 'name')) {
        return;
    }
...
//if ( false )
	{   $event = new stdClass();
        ...
        $event->description  = "$evaluation->intro"; //format_module_intro('evaluation', $evaluation, $evaluation->coursemodule, false);
 ...
 */
function mod_evaluation_core_calendar_provide_event_action(calendar_event $event, \core_calendar\action_factory $factory, int $userid = 0 )
{
    global $USER, $DB;
    if (empty($userid)) {
        $userid = $USER->id;
	...
	{	// The user can't complete the evaluation so there is no action for them.
        return null;
    }
    ...
    if ( $evaluation->course == SITEID ) 
	{	// Don't show if user not participates
