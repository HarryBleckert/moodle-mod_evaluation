...
function evaluation_get_group_values($item,
    ...
    if (intval($groupid) > 0) {
        ...
        if (!empty($_SESSION['studentid'])) {
            ...
            $query = "SELECT * FROM {evaluation_value} WHERE item = $item->id AND courseid IN ($courses) $ignore_empty_select";
...
function mod_evaluation_core_calendar_provide_event_action(calendar_event $event, \core_calendar\action_factory $factory,
    ...
    if (!$evaluation = $DB->get_record('evaluation', array("id" => $event->instance))) {
        return null;
    }

    $courseid = false;
    if (!empty($parent->courseid)) {
        $courseid = $parent->courseid;
    }
    $evaluationcompletion = new mod_evaluation_completion($evaluation, $cm, $courseid, false, null, null, $userid);
    ...
    if ($actionable && $evaluationcompletion->is_already_submitted($courseid)) {
