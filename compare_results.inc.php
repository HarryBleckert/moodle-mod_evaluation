...
function evaluation_compare_results($evaluation, $courseid = false,
    ...
    if ($allSelected == "allDepartments"  ) {
        ...
        foreach ($allResults as $allResult) {
            ...
            /*if (defined('EVALUATION_OWNER') ||
                    ($cosPrivileged ?!empty($_SESSION['CoS_privileged'][$USER->username][$allResult->course_of_studies])
                            :evaluation_is_my_cos($myEvaluations, $allResult->course_of_studies))) {*/
            if ( ($cosPrivileged ?!empty($_SESSION['CoS_privileged'][$USER->username][$allResult->course_of_studies])
                            :evaluation_is_my_cos($myEvaluations, $allResult->course_of_studies))) {
    ...
    if (is_siteadmin()) {
        ...
        if (!$filter AND $displayedResults > 1 and $displayedResults < $evaluationResults) {
            ...
            print  '<tr><td style="text-align:left;">'
				...
				<td>' . array_sum($allCounts) - $omittedResults . '</td>
				<td style="text-align:left;"><span id="omittedSubject">&nbsp;</span></td>
				<td><span id="omittedSubj">&nbsp;</span></td></tr>' . "\n";
        }
    }
    ...
    if ($qSelected) {
        ...
        */
        ...
        if ($allKey and (safeCount( $allCounts) - $omittedSubjects) >= $maxCharts) {
