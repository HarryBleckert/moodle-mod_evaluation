...
if ($teacherid) {	$urlparams['teacherid'] = $teacherid; $teacherid = 0; }
$course_of_studies = false;
if ($course_of_studiesID) {	$urlparams['course_of_studiesID'] = $course_of_studiesID; }
...
elseif ( $evaluation->course != SITEID AND !$courseid )
{	$courseid = $evaluation->course; $urlparams['courseid'] = $courseid; }
...
{	$evaluation_is_WM_disabled = (evaluation_is_WM_disabled($evaluation)?" Ausgenommen sind Weiterbildende Master Studiengänge." :"");
	$msg_privPersons = "Guten Tag " . trim($USER->firstname) . " " .trim($USER->lastname) . "<br>\n"
						...
						. ( !empty($_SESSION['CoS_privileged'][$USER->username]) 
							?'<span style="font-weight:600;white-space:pre-line;text-decoration:underline;" title="'
								.implode("\n", $_SESSION['CoS_privileged'][$USER->username]) . "\">Ihrer Studiengänge einsehen und herunterladen.</span><br>\n"
...
{	$all_courses = true;	
	$CourseTitle = "\n<span style=\"font-size:12pt;font-weight:bold;display:inline;\">".get_string("all_courses","evaluation")."</span><br>\n"; 
}
elseif ($courseid )
		...
		{	$numToDo = $numStudents * $divisor;
			...
			{	if ( $numTeachers >1 ) 
				{	$completed = round( ( $completed / $numStudents ) * 100,1) . "%"; 
					...
					print "Es wurden ".evaluation_number_format($completed_responses)." von maximal ".evaluation_number_format($numToDo)
...
{	// make get_string!
	...
	{	if ( $courseid AND !$course_has_user_participated ) 
		{	// Show description.
	...
	elseif ( !$isStudent AND $isTeacher AND $SiteEvaluation )
...
if ( defined('EVALUATION_OWNER') OR $isPermitted OR has_capability('mod/evaluation:edititems', $context) ) 
{
	...
	{	$timeopen = ($evaluation->timeopen>0)  ?$evaluation->timeopen  :(time()-80600);
		...
		{	$_SESSION["participating_courses_of_studies"] = $courses_of_studies;
			...
			{	$_SESSION["participating_courses_of_studies"] = safeCount(evaluation_get_course_of_studies_from_courseids($courses_filter)); }
		}
		...
		if ( $cosPrivileged AND $_SESSION["participating_courses_of_studies"]>1 AND $cosStudies < $_SESSION["participating_courses_of_studies"] )
		...
		echo $view_daily_link . evaluation_number_format($completed_responses) ."/" .evaluation_number_format($possible_evaluations)
		...
		elseif ( !$teamteaching AND $duplicated_replies )
		...
		if ( $evaluation->course == SITEID )
		{	echo "<b>".get_string('participants',"evaluation")."</b>: ".evaluation_number_format($_SESSION["distinct_users"]) . "/" .
				evaluation_number_format($participating_students). evaluation_calc_perc($_SESSION["distinct_users"],$participating_students)
			...
			{	$teacherslnk = new moodle_url('/mod/evaluation/print.php', array("id"=>$id, "showTeacherResults"=> 3, "goBack" => "view") );
				$view_teacher_results = html_writer::link($teacherslnk, $previewimg ); 
				$evaluated_teachers = html_writer::link($teacherslnk,"<b>".get_string('evaluated_teachers',"evaluation")."</b>: " );
				...
				{	echo 	" <b ".'title="Bereinigt: Nur Lehrende, die vor Abschluss der Evaluation Moodle nutzten"'
							...
							."<b>".evaluation_calc_perc( $_SESSION["evaluated_teachers"], $participating_active_teachers ) ."</b>";
				}
				echo " ".$view_teacher_results."<br>\n";  
			}
			else
			{	echo "<b>".get_string('teachers_in_courses',"evaluation")."</b>: ".evaluation_number_format($participating_teachers)
					."<br>\n"; 
			}
			...
			{	echo "<b title=\"Je eine Abgabe pro Dozent_in ist aktiviert?\">".get_string("teamteaching","evaluation")."</b>: " 
					...
					. ((!$teamteaching AND $duplicated_replies) ?" - Duplizierte Abgaben: " . evaluation_number_format( $duplicated_replies ) :"")
					."<br>\n"; 
			}

			$previewlnk->remove_params("daily_progress");
			$participating_courses_of_studies = html_writer::link($previewlnk,"<b>".get_string('participating_courses_of_studies',"evaluation")."</b>: " );
			...
			$logtitle 	= "Auswertung des Moodle Logs. Log Daten werden ". get_config('logstore_standard')->loglifetime . " Tage aufbewahrt.";
			echo html_writer::link($usagelink, "<b>".get_string('usageReport',"evaluation")."</b> ",array( 'title' => $logtitle ) )
												." ".$view_usageReport ."<br>\n";
		}
	}
	else
	{	echo "<b>".get_string('completed_evaluations',"evaluation")."</b>: ".evaluation_number_format($completed_responses) ."<br>\n"; 
		if ( !$courseid )
		{	echo "<b title=\"Je eine Abgabe pro Dozent_in ist aktiviert?\">".get_string("teamteaching","evaluation")."</b>: " 
...
{	$total 		= total_evaluation_days($evaluation);
	...
	echo "<b>".get_string('evaluation_period','evaluation') ."</b>: " . date("d.m.Y",$evaluation->timeopen) . " - ".date("d.m.Y",$evaluation->timeclose) . 
...
{	if ( isset($_GET['shuffle']) )
	...
	*/
	...
	{	//echo "<br><hr>evaluation_set_results( evaluation, false, true ) ...<hr>";
		...
		echo "<br>isEvaluationCompleted( $evaluation->id, $courseid, $USER->id ): " . var_export(isEvaluationCompleted( $evaluation, $courseid, $USER->id ),true);
...
if ($evaluationcompletion->can_complete()) {
    ...
    if (!$evaluationcompletion->is_open() AND !($isPermitted OR defined('EVALUATION_OWNER')) ) {
		...
		echo "<b>Die Teilnahme an der Evaluation ".($evaluation->timeclose<time()?"war":"ist")." vom ".date("d.m.Y",$evaluation->timeopen)." bis zum ".date("d.m.Y",$evaluation->timeclose)." möglich.</b><br>\n";
		if ( $is_open )
        {	echo $OUTPUT->continue_button(course_get_url($courseid ?: $course->id)); }
    } 
	elseif ($evaluationcompletion->can_submit()) 
	{	
		...
		{	if ( !$SiteEvaluation OR ($courseid ) ) 
			{	// Display a link to complete evaluation or resume.
				...
				echo html_writer::div(html_writer::link($completeurl, $label, array('class' => 'btn btn-secondary')), 'complete-evaluation');
			}
		}
    } else {
        ...
        {	if ( $isStudent )
			{	if ( $completed_all ) //"Guten Tag $fullname<br>" .
				...
				elseif ( $courseid AND evaluation_has_user_participated($evaluation, $USER->id, $courseid ) )
				{ echo "Guten Tag $fullname<br>" .'<b style="color:darkgreen;">Sie haben für diesen Kurs bereits an der Lehrevaluation teilgenommen!</b><br>'; }
			}
		} 
		else
...
elseif ( $evaluationcompletion->is_open() ) 
...
if (  !($isPermitted OR defined('EVALUATION_OWNER')) AND empty($_SESSION["myEvaluations"]) )
//if (  empty($_SESSION["myEvaluations"]) )
{ 	if ( !$is_open AND $isEnrolled )
	...
	elseif ( $SiteEvaluation AND !$isEnrolled )
...
elseif ( $evaluation->course == SITEID )
{	if ( ( $isTeacher AND !$isStudent ) OR ( $teacheridSaved>0 AND defined('EVALUATION_OWNER') ) )
	...
	elseif ( !empty($_SESSION["myEvaluations"]) ) 
...
{	print "<br>Evaluation->id: $evaluation->id";
	if ( $SiteEvaluation AND ( !isset($_SESSION["make_block_evaluation_visible"]) OR $_SESSION["make_block_evaluation_visible"] !== $evaluation->name ) )
